...
- The script requires Python 3.10+ by default because older interpreters cannot parse `from __future__ import annotations` and are unsupported by current PyTorch/Triton. Configure `PYTHON_MODULE` and/or `PYTHON_COMMAND`; loading a newer module does not upgrade an existing `.venv-aspire`, so use a new `VENV_DIR` or manually recreate a stale old venv.
- Default `PYTHON_MODULE` is `pytorch/2.10.0-py3-cu12.6`. That Aspire module supplies torch, numpy (likely), triton, and CUDA 12.6. The venv is created with `--system-site-packages` so the module's runtime is reused; pip only adds packages the module omits and never reinstalls torch over it.
