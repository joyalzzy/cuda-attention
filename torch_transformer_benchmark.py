...
# Optional custom Triton kernels. Absent or broken imports must never break
# the harness: every kernel entry point is additionally guarded at call time
# by dispatch checks and try/except fallbacks.
try:
    from kernels import dispatch as _kernel_dispatch
    from kernels import triton_attention as _kernel_attention
    from kernels import triton_norm as _kernel_norm
    from kernels import triton_ffn as _kernel_ffn
except Exception:  # pragma: no cover - kernels are an optional accelerator
    _kernel_dispatch = None
...
class UserOptimizedTransformer(BaselineTransformer):
    """Transformer using PyTorch scaled-dot-product attention when available.

    The module hierarchy and parameter names are inherited unchanged so strict
    state-dict copying remains valid. Unsupported runtimes use the reference
    attention implementation as a correctness-first fallback.
    """

    @staticmethod
    def _mask_has_invalid_tokens(valid_token_mask: torch.Tensor) -> bool:
        """Return whether a harness mask contains padding.

        Avoid scalar extraction while compiling: returning True builds a
        semantically equivalent additive all-zero mask and keeps the graph
        traceable. Eager all-True masks still take the faster mask-free path.
        """
        if torch.compiler.is_compiling() if hasattr(torch, "compiler") else False:
            return True
        return bool((~valid_token_mask).any().item())
    ...
    def _sdpa_attention(
        ...
        if valid_token_mask is not None:
            # The harness always supplies a mask, including an all-True mask
            # when padding is disabled. Treat that case as no mask so causal
            # attention remains eligible for the fused is_causal path.
            has_invalid_keys = UserOptimizedTransformer._mask_has_invalid_tokens(
                valid_token_mask
            )
            if not has_invalid_keys:
                valid_token_mask = None

        if valid_token_mask is not None:
            # Use an additive mask for consistent semantics across older math
            # and newer fused SDPA backends: 0 keeps a key and -inf rejects it.
        ...
        # Omitting ``scale`` keeps compatibility with older SDPA APIs. The
        # default is exactly 1/sqrt(query.size(-1)), matching the reference.
    ...
    def _forward_layer(
        ...
        # Unify the effective padding decision once, before any attention
        # execution. ``valid_token_mask=None`` and an all-True mask are both
        # "no padding" and skip mask construction and zeroing.
        ...
        if self._use_kernel_layernorm(x):
            try:
                normalized = _kernel_norm.layernorm(
                    x, layer.norm1.weight, layer.norm1.bias, layer.norm1.eps
                )
            except Exception:
                normalized = layer.norm1(x)
        x = x + self._block_attention(layer, normalized, effective_mask)

        if self._use_kernel_ffn(x, layer):
            try:
                return _kernel_ffn.fused_ffn(
                    x,
                    layer.norm2,
                    layer.ffn_in,
                    layer.ffn_out,
    ...
    def _use_kernel_layernorm(self, x: torch.Tensor) -> bool:
        return bool(
            _kernel_dispatch is not None
            and _kernel_norm is not None
            and _kernel_dispatch.choose_layernorm(x.device, x.shape[-1])
        )

    def _use_kernel_ffn(self, x: torch.Tensor, layer: BaselineTransformerBlock) -> bool:
        return bool(
            _kernel_dispatch is not None
            and _kernel_ffn is not None
            and _kernel_dispatch.choose_ffn(x.device, x.shape[-1], layer.ffn_in.out_features)
        )
    ...
    def _block_attention(
        ...
        if self._use_kernel_attention(normalized):
            try:
                ...
                # Kernel compilation or launch failure: fall through to the
                # framework paths below without aborting the run.
                pass

        use_sdpa = hasattr(F, "scaled_dot_product_attention")
        # The CPU math backend can accumulate fp16/bf16 softmax differently
        # from the fp32 reference. Use the exact fallback for those cases.
        if normalized.device.type != "cuda" and normalized.dtype in (
            torch.float16,
            torch.bfloat16,
        ):
            use_sdpa = False
        ...
        if use_sdpa:
            try:
                ...
                # Backend eligibility varies by PyTorch version, GPU, dtype,
                # head dimension, and mask. Preserve correctness if SDPA rejects
                # a supported reference case.
                pass
        return layer.attention(normalized, valid_token_mask, self.config.causal)

    def _use_kernel_attention(self, normalized: torch.Tensor) -> bool:
        return bool(
            _kernel_dispatch is not None
            and _kernel_attention is not None
            and _kernel_dispatch.choose_attention(
                normalized.device,
                normalized.shape[1],
    ...
    def _triton_attention(
        ...
        k = attention._split_heads(attention.k_proj(x))
        v = attention._split_heads(attention.v_proj(x))
        context = _kernel_attention.flash_attention(
            q,
            k,
            v,
            valid_token_mask,
    ...
    def forward(
        ...
        for layer in self.layers:
            x = self._forward_layer(layer, x, valid_token_mask)
        x = self.final_norm(x)
        if valid_token_mask is not None:
            x = x.masked_fill(~valid_token_mask[..., None], 0)
        return x
